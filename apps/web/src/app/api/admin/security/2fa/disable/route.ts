...
/**
 * Exige el código 2FA vigente para desactivarlo (no solo la sesión) — si
 * alguien roba la sesión del navegador pero no tiene el celular con el
 * autenticador, no puede bajar esta defensa por su cuenta. Rate-limited por
 * la misma razón que /2fa/confirm: sin límite, esa defensa se podría
 * romper a fuerza bruta de todos modos.
 */
export async function POST(req: NextRequest) {
  const session = await getServerSession(authOptions);
  if (!session?.user?.email) return NextResponse.json({ error: "No autorizado" }, { status: 401 });

  const { allowed, retryAfterSeconds } = checkRateLimit(`2fa-disable:${getClientIp(req)}`, {
    limit: 10,
    windowMs: 15 * 60_000,
  });
  if (!allowed) {
